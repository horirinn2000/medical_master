...
func ImportDentalPracticeSupports(db *gorm.DB) {
	importCsv(db, 25, "csv/dental_practice/01補助マスターテーブル（歯科）.csv", func(record []string) model.DentalPracticeSupport {
		return model.DentalPracticeSupport{
			...
			InclusionUnit1:               record[4],
			InclusionGroup1:              record[5],
			InclusionUnit2:               record[6],
			InclusionGroup2:              record[7],
			InclusionUnit3:               record[8],
			InclusionGroup3:              record[9],
			ConflictPerDay:               record[10],
			ConflictSameMonth:            record[11],
			ConflictSimultaneous:         record[12],
			ConflictSameSiteSimultaneous: record[13],
			ConflictPerWeek:              record[14],
			ConflictReserved:             record[15],
			CalculationCountRelation:     record[16],
			Reserved1:                    record[17],
			Reserved2:                    record[18],
			Reserved3:                    record[19],
			Reserved4:                    record[20],
