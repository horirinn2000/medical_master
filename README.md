...
本システムのAPI仕様およびデータ構造は、プロジェクト内の `/doc` ディレクトリに格納されているPDF仕様書に基づいています。

診療報酬改定（4月、6月など）によりマスターファイルの構造が変更される可能性があります。
`/doc` 内のPDF仕様書が古い場合、本システムはまだ最新の改定に対応していない可能性がありますのでご注意ください。
